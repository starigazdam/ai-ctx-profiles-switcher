...
Following the empirical test above (confirmed: Copilot CLI's write-tmp+rename on `settings.json` replaces the `COPILOT_HOME` symlink with a plain file, orphaning the real `~/.copilot/settings.json`), the design history document has been revised:
...
The full historical design is in `design-history-copilot-home.md` (sections 3.4a, 5.2 items 12–14, checklist steps 3/7/11, and the Q1 rewrite in section 9).
